    ...
    request(options, (error, response, body) => {
      if(error){console.log(error);}
        ...
        console.log(definitions);
          //message.channel.sendMessage(`**${word}:**\n${definition} \n\uD83D\uDC4D ${thumbsup} \uD83D\uDC4E ${thumbsdown} \n \nExample: ${example}`);
          let definition = "";
        for(i=0;i<dictResponse.results[0].lexicalEntries[0].entries.length;i++){
        definition += `${i+1}. ${dictResponse.results[0].lexicalEntries[0].entries[i].senses[0].definitions} \n`
        }
        const embed = new Discord.RichEmbed()
            .setAuthor(`${word}`)

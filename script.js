...
const commands = {
	'play': (msg) => {
		...
		(function play(song) {
			...
			dispatcher = msg.guild.voiceConnection.playStream(yt(song.url, { audioonly: true }), { passes : apasses });
	...
	'add': (msg) => {
		...
		if (url == '' || url === undefined) return msg.channel.sendMessage(`You must add a url, or youtube video id after ${musicPrefix}add`);
	...
	'reboot': (msg) => {
		if (msg.author.id == '186693404288090114') process.exit(); 
	}
};
const monthNames =  [
  "Jan", "Feb", "Mar",
